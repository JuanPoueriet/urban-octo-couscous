...
export class ProductDetail implements OnInit, OnDestroy {
  ...
  public relatedProducts$: Observable<Product[]> | undefined;
  ...
  ngOnInit(): void {
    ...
    this.relatedProducts$ = this.route.paramMap.pipe(
      switchMap(params => {
        const slug = params.get('slug');
        return slug ? this.dataService.getRelatedProducts(slug) : of([]);
      })
    );
  ...
  private updateMetadata(product: Product): void {
    ...
    this.translate.get([titleKey, descKey, 'COMMON.BREADCRUMB_HOME', 'HEADER.PRODUCTS', 'COMMON.DEFAULT_DESCRIPTION']).subscribe(translations => {
      ...
      const productSchema = {
        ...
        'image': `${baseUrl}/assets/imgs/jsl-social-default.jpg`,
        'sku': `JSL-${product.key.toUpperCase()}`,
        'brand': {
          '@type': 'Brand',
          'name': 'JSL Technology'
        },
